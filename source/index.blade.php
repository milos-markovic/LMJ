...
@section('body')
    ...
    <section>
        @foreach($posts->where('type','feature') as $post)
            <img src="{{ $post->image }}" alt="feature post">
    ...
    <section>
        @foreach($posts->where('type','regular') as $post)
            ...
            <div x-data="{}" class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-8">
            
                @foreach( $post->images as $img)
                <div>
                    <a @click="$dispatch('img-modal', {  imgModalSrc: '{{ $img['url'] }}', imgModalDesc: '{{ $img['title'] }}' })" class="cursor-pointer">
                        <img src="{{ $img['url'] }}" alt="{{ $img['title'] }}" class="border border-gray-400 p-1">
                    </a>
                    <p class="text-sm m-0 mt-1">{{ $img['title'] }}</p>
                </div>
                @endforeach
    ...
    <div class="border-t-2 border-gray-200 mt-16 mb-10"></div>

    <section>
        @foreach($posts->where('type','images') as $post)
            <h2 class="font-normal mb-10">{{ $post->title }}</h2>
